...
//创建简单模式的RabbitMQ实例(消息没有持久化)
func NewSimpleRabbitMQ(url string, queueName string) (*rabbitMQ, error) {
	return newRabbitMQ(url, queueName, "", "", simple)
}

//简单模式：生产者客户端
func (this *rabbitMQ) simpleSend(msg string) (err error) {
...
func (this *rabbitMQ) simpleConsume(handleFunc HandleFunc) (err error) {
	queue, err := this.channel.QueueDeclare(
		this.queueName, //队列名称
		false,          //消息持久化
		false,          //是否自动删除

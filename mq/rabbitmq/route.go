...
//创建路由模式下的RabbitMQ实例(消息持久化，手动应答)
func NewRouteRabbitMQ(url, exchange, routingKey string) (*rabbitMQ, error) {
	return newRabbitMQ(url, "", exchange, routingKey, route)
}

func (this *rabbitMQ) routeSend(msg string) (err error) {
	//声明交换机
...
//路由模式：消费者客户端
//路由模式：消费者客户端
func (this *rabbitMQ) routeConsume(handleFunc HandleFunc) (err error) {
	//声明交换机
	err = this.channel.ExchangeDeclare(
		this.exchange,
		"direct",
